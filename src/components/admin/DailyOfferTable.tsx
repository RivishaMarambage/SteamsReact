...
import { MoreHorizontal, PlusCircle, Calendar as CalendarIcon, Tag, Search, FilterX, ChevronRight, X, Clock } from 'lucide-react';
...
export default function DailyOfferTable() {
  ...
  const filteredOffers = useMemo(() => {
    ...
    return offers.filter(offer => {
      ...
      const matchesSearch = offer.title.toLowerCase().includes(searchTerm.toLowerCase()) || itemNames.includes(searchTerm.toLowerCase());
  ...
  const handleToggleCategory = useCallback((categoryName: string) => {
    ...
    setFormData(prev => {
      ...
      if (allSelected) {
        return { ...prev, menuItemIds: currentlySelected.filter(id => !itemIds.includes(id)) };
      } else {
        return { ...prev, menuItemIds: Array.from(new Set([...currentlySelected, ...itemIds])) };
      }
    });
  }, [groupedMenuItems]);
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        ...
        setFormOpen(false);
        setSelectedOffer(null);
        toast({ title: "Offer Saved", description: "Promotion configured successfully." });
    } catch (error: any) {
    ...
    <Card className="shadow-lg">
      <CardHeader className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative w-full md:w-64">
            ...
            <Input placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 h-10 rounded-full" />
          </div>
          <select 
          ...
          <Button size="sm" onClick={handleAddNew} className="h-10 ml-auto rounded-full px-6 shadow-lg"><PlusCircle className="mr-2 h-4 w-4" /> New Offer</Button>
        </div>
      </CardHeader>
          ...
          <TableBody>
            {filteredOffers.map(offer => (
              <TableRow key={offer.id}>
                ...
                <TableCell className="text-xs font-mono text-muted-foreground">{offer.offerStartDate} to {offer.offerEndDate}</TableCell>
      ...
      <Dialog open={isFormOpen} onOpenChange={setFormOpen}>
        <DialogContent className="sm:max-w-3xl h-[90vh] flex flex-col p-0 overflow-hidden rounded-[2.5rem]">
          <form onSubmit={handleFormSubmit} className="flex flex-col h-full w-full overflow-hidden">
            <div className="p-8 border-b bg-muted/10 shrink-0">
              <DialogHeader>
                <DialogTitle className="font-headline text-3xl uppercase tracking-tighter text-primary">Configure Promotion</DialogTitle>
                <DialogDescription className="font-bold text-[10px] uppercase tracking-widest text-muted-foreground">Manage multi-item rewards and tier savings.</DialogDescription>
              </DialogHeader>
            </div>
            
            <div className="flex-1 overflow-y-auto min-h-0 p-8 space-y-12">
              <div className="grid gap-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="grid gap-3">
                    <Label className="text-xs font-black uppercase tracking-widest text-muted-foreground">Validity Range</Label>
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant="outline" type="button" className="w-full justify-start text-left font-mono h-14 rounded-2xl px-6 border-2">
                  ...
                  <div className="grid gap-3">
                    <Label className="text-xs font-black uppercase tracking-widest text-muted-foreground">Support Type</Label>
              ...
              <div className="space-y-8">
                <div className="flex justify-between items-center">
                  <h3 className="text-2xl font-headline font-black uppercase tracking-tighter">Menu Selection</h3>
                  <div className="flex gap-2">
                    <Button type="button" variant="outline" size="sm" className="rounded-full text-[10px] uppercase font-black" onClick={handleSelectAll}>Select All</Button>
                    <Button type="button" variant="ghost" size="sm" className="rounded-full text-[10px] uppercase font-black text-destructive" onClick={handleDeselectAll}>Clear</Button>
                  </div>
                </div>
                  ...
                  {Object.entries(groupedMenuItems).map(([categoryName, items]) => {
                    ...
                    return (
                      <AccordionItem key={categoryName} value={categoryName} className="border-2 rounded-[1.5rem] overflow-hidden bg-background">
                        <div className="flex items-center justify-between w-full bg-muted/5 pr-4 border-b">
                          <AccordionTrigger className="flex-1 px-6 h-16 hover:no-underline">
                          ...
                          <span 
                            className="h-8 text-[9px] font-black uppercase tracking-[0.1em] px-4 rounded-full bg-primary/10 hover:bg-primary/20 text-primary transition-colors flex items-center cursor-pointer select-none"
                            onClick={(e) => { 
                              e.preventDefault(); 
                              e.stopPropagation(); 
                        ...
                        <AccordionContent className="p-6">
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {items.map(item => {
                              ...
                              return (
                                <div key={item.id} className={cn("flex items-center space-x-4 p-4 rounded-2xl border-2 transition-all cursor-pointer", isChecked ? "bg-primary/5 border-primary" : "border-muted/50 hover:border-primary/20")} onClick={() => handleToggleItem(item.id)}>
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <span className="text-[10px] font-bold text-muted-foreground">LKR {item.price.toFixed(0)}</span>
                                  </div>
                                </div>
                              );
                            })}
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                  ...
                  <Tabs value={formData.discountType} onValueChange={(v) => setFormData(p => ({ ...p, discountType: v as any }))}>
                    <TabsList className="grid w-full grid-cols-2 bg-white/50 p-1 rounded-full h-12">
                      <TabsTrigger value="fixed" className="rounded-full text-[10px] font-black uppercase">LKR</TabsTrigger>
                      <TabsTrigger value="percentage" className="rounded-full text-[10px] font-black uppercase">Percent</TabsTrigger>
                    </TabsList>
                  </Tabs>
                </div>
            ...
            <div className="p-8 border-t bg-muted/10 flex flex-col sm:flex-row gap-4 shrink-0">
              <Button type="button" variant="ghost" onClick={() => setFormOpen(false)} className="flex-1 h-16 rounded-full font-black uppercase tracking-widest">Discard</Button>
              <Button type="submit" className="flex-1 h-16 rounded-full font-black uppercase tracking-widest shadow-2xl">Save Promotion</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
