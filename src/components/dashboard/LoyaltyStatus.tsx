...
export default function LoyaltyStatus({ user }: { user: User }) {
  const currentTier = getLoyaltyTier(user.points);
  const Icon = currentTier.icon;
  const progress = currentTier.nextTierPoints
    ? ((user.points - currentTier.minPoints) / (currentTier.nextTierPoints - currentTier.minPoints)) * 100
    : 100;
    ...
    <Card className="shadow-lg">
      ...
      <CardContent>
        <div className="space-y-2">
          <div className="text-3xl font-bold text-primary">{user.points} Points</div>
          <Progress value={progress} className="h-3 [&>*]:bg-primary" />
          <div className="text-sm text-muted-foreground">
            {currentTier.nextTierPoints ? (
              <span>
                You are <strong>{currentTier.nextTierPoints - user.points}</strong> points away from the <strong>{LOYALTY_TIERS.find(t => t.minPoints === currentTier.nextTierPoints)?.level}</strong> tier.
              </span>
            ) : (
